...
// NOTE: Full path resolution is done in user-space via /proc/<pid>/fd
// because dentry chain walks exceed BPF verifier complexity limits.
struct file_event_t {
    u32 pid;            // Process ID
    u32 uid;            // User ID
...
// Per-CPU map to hold event struct (exceeds 512-byte BPF stack limit)
BPF_PERCPU_ARRAY(event_buf, struct file_event_t, 1);
...
int trace_vfs_write(struct pt_regs *ctx,
    ...
    int zero = 0;
    struct file_event_t *event = event_buf.lookup(&zero);
    if (!event)
        return 0;
...
class EBPFMonitor:
    ...
    @staticmethod
    def _resolve_path(pid: int, filename: str) -> str:
        """
        Resolve the full path of a file being accessed by a process.

        Reads /proc/<pid>/fd/ symlinks to find an open fd pointing to
        a file with the matching basename. This is more reliable than
        dentry chain walks in BPF which exceed verifier complexity limits.

        Args:
            pid:      Process ID
            filename: Basename of the file (from dentry)

        Returns:
            Full path if resolved, empty string otherwise.
        """
        try:
            ...
            for fd in os.listdir(fd_dir):
                try:
                    ...
                    if link.endswith(f"/{filename}") or link == filename:
                        return link
